...
import { fetchActiveCourses } from "../../config/supabaseClient";
...
export default function HomeScreen() {
  ...
  const activeCourses = fetchActiveCourses('802201628');
  ...
  if (activeCourses != null) {
    for (var i = 0; i < activeCourses.length && i < 10; i++) {
      const course = {
        courseImage: require('../../assets/data-structures.png'),
        courseName: String(activeCourses[i].course),
        courseTutor: String(activeCourses[i].tutor_name),
        link: 'www.google.com',
        startDate: '9/25/2023',
        endDate: '10/20/2024',
        startTime: '1:00pm',
  ...
  const courseData = status === "Active" ? active_courses : test_course_card_past;
